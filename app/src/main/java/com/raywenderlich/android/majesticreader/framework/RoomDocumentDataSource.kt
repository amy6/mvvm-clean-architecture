...
import data.DocumentDataSource
...
class RoomDocumentDataSource(val context: Context) : DocumentDataSource {
    ...
    override suspend fun remove(document: Document) = documentDao.removeDocument(DocumentEntity(document.url, document.name, document.size, document.thumbnail))
}